...
# 화면 타이틀 설정
pygame.display.set_caption("Dano Game") # 게임 이름
...
# 캐릭터(스프라이트) 불러오기
character = pygame.image.load("C:/Users/LEE/Desktop/GitHubs/Python/pygame/character.png")
character_size = character.get_rect().size  # 이미지의 크기를 구해옴
character_width = character_size[0]    # 캐릭터의 가로 크기
character_height = character_size[1]   # 캐릭터의 세로 크기
character_x_position = (screen_width / 2) - (character_width / 2)   # 화면 가로의 절반 크기에 위치  (가로 위치)
character_y_position = screen_height - character_height    # 화면 세로 크기 가장 아래에 위치 (세로 위치)
...
# 빌런 캐릭터
enemy = pygame.image.load("C:/Users/LEE/Desktop/GitHubs/Python/pygame/enemy.png")
enemy_size = enemy.get_rect().size  # 이미지의 크기를 구해옴
enemy_width = enemy_size[0]    # 캐릭터의 가로 크기
enemy_height = enemy_size[1]   # 캐릭터의 세로 크기
enemy_x_position = (screen_width / 2) - (enemy_width / 2)   # 화면 가로의 절반 크기에 위치  (가로 위치)
enemy_y_position = (screen_height / 2) - (enemy_height / 2)    # 화면 세로 크기 가장 아래에 위치 (세로 위치)
...
# print("fps :" + str(clock.get_fps()))
    ...
    # 충돌 처리를 위한 rect 정보 업데이트  rect == retangle
    ...
    # 충돌 체크
    if character_rect.colliderect(enemy_rect): # 캐릭터가 빌런과 충돌을 했느냐
        print("충돌 했어요")
        running = False

    screen.blit(background, (0, 0)) # 배경 그리기
    screen.blit(character, (character_x_position, character_y_position)) # 캐릭터 그리기
    screen.blit(enemy, (enemy_x_position, enemy_y_position)) # 빌런 그리기
